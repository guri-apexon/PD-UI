...
describe("Add Protocol Test Suite", () => {
 test("Should render AddProtocol Component", async () => {
  ...
  test("Should render AddProtocol Component with Modal", async () => {
    ...
    let protcolNumber= getByTestId('protocol-number-texfield').children[1].children[0];
    let versionNumber= getByTestId('version-number-texfield').children[1].children[0];
  ...
  test("Should Save Post Correctly", async () => {
    ...
    let protcolNumber= container.getByTestId('protocol-number-texfield').children[1].children[0];
    fireEvent.change(protcolNumber, {target:{value:'a'}});
    expect(protcolNumber.value).toBe('a');
    let savemodal=container.getByTestId('add-protocol-modal').children[2].children[0].children[2].children[0].children[1]
...
function selectAutoComplete(Container) {
  ...
  const autocompleteDocumentStatus = Container.getByTestId('document-status-texfield');
