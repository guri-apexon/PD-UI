...
import { handleQCAdd } from "./utils-new";
...
export function* updateDataStream(action) {
  const { type, lineId, sectionName, contentID } = action.payload;
  console.log("Action Payload", action.payload);
  const currentData = yield getWrapperState();
  let cloneData = cloneDeep(currentData);

  let arrToUpdate = cloneData.data[sectionName].detail;
  const updatedSectionArr = handleQCAdd(arrToUpdate, lineId, type, contentID);
  console.log("Updated data", updatedSectionArr);
  cloneData.data[sectionName].detail = updatedSectionArr;
  ...
  //     }
  //   }
  console.log("Updated data", cloneData);
  yield put(getWrapperData(cloneData));
}
