...
describe('PDTable component', () => {
  ...
  test('handles the delete cancel action', () => {
  ...
  test('handles the delete success action', () => {
  ...
  test('should dispatch a CONTENT_UPDATE event when activeLineID is not equal to lineID and tableSaved is false', () => {
  ...
  test('fires handlechange event when contnent in the cell changes', async () => {
    const tableData = {
      TableProperties: [
        {
          row_indx: 0,
    ...
    const abText = screen.getByText('ab');
    expect(abText).toBeInTheDocument();
    fireEvent.doubleClick(screen.getByText('ab'));
    await fireEvent.input(abText, {
      target: { innerHTML: 'abcd' },
    });
    const abcdText = screen.getByText('abcd');
    expect(abcdText).toBeInTheDocument();
    fireEvent.blur(abcdText);
