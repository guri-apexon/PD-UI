...
function DipaView() {
  ...
  const deleteSegment = (segmentId) => {
    const getUpdatedMetadata = (array) =>
      array.map((section) => ({
        ...section,
        derive_seg: section.derive_seg?.find((seg) => seg.ID === segmentId)
          ? section.derive_seg?.filter((seg) => seg.ID !== segmentId)
          : section.derive_seg,
      }));
    setMetadata([...getUpdatedMetadata(metadata)]);
  };
  const updateSegment = (segmentId, segment) => {
    const getUpdatedMetadata = (array) =>
      array.map((section) => ({
        ...section,
        derive_seg: section.derive_seg?.map((seg) => ({
          ...seg,
          derive_seg: seg.ID === segmentId ? segment : seg.derive_seg,
        })),
      }));
    setMetadata([...getUpdatedMetadata(metadata)]);
  };
  ...
  return (
    <div>
      <div className="container" data-testid="dipa-view-sample">
