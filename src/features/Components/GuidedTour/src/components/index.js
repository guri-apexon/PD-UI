...
class Joyride extends React.Component {
  ...
  static propTypes = {
    ...
    closeTourHandler: PropTypes.func,
  ...
  componentDidMount() {
    ...
    /* if (closeButtonClicked && run) {
      this.props.closeTourProp = true;
    } */
  ...
  closeTourHandler = data => {
    const { closeTourHandler } = this.props;

    /* istanbul ignore else */
    if (is.function(closeTourHandler)) {
      closeTourHandler(data);
    }
  }
    ...
    if (status === STATUS.RUNNING && step) {
      output = (
        <Step
          ...
          closeTourHandler={this.closeTourHandler}
        />
      );
    }
